...
import { 
  ...
  Menu, X, Globe, ChevronRight, Sparkles, Calendar as CalendarIcon 
} from 'lucide-react';
    ...
    return (
      <Link to={to} onClick={() => setIsMobileMenuOpen(false)}>
        <div className={`relative flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-300 group overflow-hidden ${
          isActive 
            ? 'text-white shadow-lg shadow-blue-900/40 bg-blue-600/20' 
            : 'text-slate-400 hover:text-white hover:bg-white/5'
        }`}>
          {isActive && (
            <motion.div layoutId="activeTab" className="absolute left-0 top-0 bottom-0 w-1 bg-blue-500" initial={{ opacity: 0 }} animate={{ opacity: 1 }} />
          )}
    ...
    <div className="h-screen bg-[#F8FAFC] flex font-sans overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#001F54] text-white transition-transform duration-300 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static flex flex-col shadow-2xl`}>
        ...
        <div className="p-6 space-y-2 flex-1 overflow-y-auto">
          ...
          {/* YENİ EKLENEN TAKVİM */}
          <SidebarLink to="/panel/calendar" icon={CalendarIcon} label="Takvim & Ajanda" />
        ...
        <div className="p-6 border-t border-white/5 bg-[#001845] shrink-0">
          <Link to="/" className="flex items-center gap-2 text-slate-400 hover:text-white text-xs mb-4 px-2 transition-colors"><Globe size={14} /> Web Sitesine Dön</Link>
          <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 bg-white/5 text-red-300 hover:bg-red-600 hover:text-white px-4 py-3 rounded-xl transition-all text-sm font-bold border border-white/5 hover:border-transparent"><LogOut size={18} /> Çıkış Yap</button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 relative h-full">
        <header className="lg:hidden bg-white/80 backdrop-blur-md h-16 border-b border-slate-200 flex items-center justify-between px-4 sticky top-0 z-40 shrink-0">
           ...
           <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-slate-600 p-2 rounded-lg hover:bg-slate-100">{isMobileMenuOpen ? <X size={24}/> : <Menu size={24}/>}</button>
        </header>
